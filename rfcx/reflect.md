...
- Method result caching is undefined: `reflect` does *not* guarantee that any method call made will cache/'memoize' the returned output. That is to say: `AstDocument:comments()` may or may not return the same table reference as a second call to `AstDocument:comments()`.
